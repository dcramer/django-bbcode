...
[/table][/code]
    ...
    close_pattern = re.compile('\[/table\]', re.I)
...
[/row][/code]
    """
    open_pattern = re.compile(patterns.no_argument % 'row', re.I)
    close_pattern = re.compile(patterns.closing % 'row', re.I)
...
[col=<colspan>]text[/col][/code]
    ...
    """
    open_pattern = re.compile(r'(\[col\]|\[col="?(?P<argument>[^]]+)?"?\])', re.I)
    close_pattern = re.compile(patterns.closing % 'col', re.I)
...
[head=<colspan>]text[/head][/code]
    ...
    [i]colspan[/i]: must be digit. Default: 1 (normal)
    """
    open_pattern = re.compile(patterns.single_argument % 'head', re.I)
    close_pattern = re.compile(patterns.closing % 'head', re.I)
