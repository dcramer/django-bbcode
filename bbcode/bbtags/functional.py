...
[url=$myvar$/someimg.png]super cool picture[/url][/code]
    """
    open_pattern = re.compile(patterns.no_argument % 'def', re.I)
    close_pattern = re.compile(patterns.closing % 'def', re.I)
...
[/args][/code]
    ...
    """
    open_pattern = re.compile('\[args(?P<args>(=[^\]]+)| ([^\]]+))\]', re.I)
    close_pattern = re.compile(patterns.closing % 'args', re.I)
...
[img]http://www.mysite.com/img_010.png[/img][/code]
    ...
    close_pattern = re.compile(patterns.closing % 'range', re.I)
    verbose_name = 'Range'
